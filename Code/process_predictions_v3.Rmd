...
ref_result_RO[["mask_area_results"]] <- ref_result_RO[["mask_area_results"]]/0.85
...
#Count of points per species
```{r Count of points per species}
sp_count <- ref_result_RO %>% 
  group_by (scientificName) %>% 
  summarise (num_samples = n())

write.csv(sp_count, file = "~/Uni/Honours/Thesis/Data Analysis or Code/Plots/species_count.csv")
```
...
p1 <- ggplot(data=ref_result_RO, mapping = aes(y = ..density.., x=log_MAR)) +
  geom_histogram(bins = 50) + #, aes(fill=Subgenus), position="dodge") +
  theme_bw() + xlab(expression(paste("Logged leaf size"))) 
  
p2 <- ggplot(data=ref_result_RO, mapping = aes(y = ..density.., x=log_CAR)) +
  geom_histogram(bins = 50) + #, aes(fill=Subgenus), position="dodge") +
  theme_bw() + xlab(expression(paste("Logged largest in-circle area")))
...
```{r Repeating analysis on AusTraits - Trait x Climate}
  # This chunk of code extracts the leaf area measurements of eucalypts from the AusTraits database (v4.0.0). From here using its latitude and longitude of their collection site, their respective climatic variables are appended to create climate x trait plots
...
p1.5 <- ggplot () +
  ...
  geom_jitter(data = Wrights_data_eucs, mapping = aes (x=Prec, y=mask_area_results), colour = "#E69F00", width = 0.1, height = 0.1, size = 1) +
  geom_smooth(method = 'lm', data = Wrights_data_eucs, mapping = aes (x=Prec, y=mask_area_results), colour = "Brown", linewidth=0.5) +
  labs(y="Leaf area (cm2)", x = "Mean annual precipitation (mm)") +
  theme_bw() +
  theme (legend.position="none") +
  scale_y_continuous(trans='log10') +
...
##Comparing AusTraits spread to ours - density + frequency
```{r Comparing different datasets spread to ours - density + frequency}
...
p1 <- ggplot (data = ref_result_RO, mapping = aes (x=Temp, y=mask_area_results)) +
  geom_point(alpha = 0.02, color = "#E69F00") +
...
ggsave(filename="~/Uni/Honours/Thesis/Data Analysis or Code/Plots/Basic_Climate_Trait_PlotThesis.jpg",plot=result, units = "px", height = 2100, width = 2300, limitsize = FALSE)
...
p1 <- ggplot (data = ref_result_RO, mapping = aes (x=Temp, y=mask_area_results)) +
  ...
  labs(y="Leaf area (cm2)", x = expression(paste("Mean annual temperature ("^"o", "C)"))) +
  theme_bw() +
  theme (legend.position="none") +
  scale_y_continuous(trans='log10') +
  geom_abline(slope = 0.043, intercept = 0.207, linetype="dashed", 
...
p1 <- ggplot (data = ref_result_RO, mapping = aes (x=Temp, y=circle_area_results)) +
  geom_jitter(aes(color=scientificName), alpha = 0.1, width = 0.1, height = 0.1, size = 0.2) +
  geom_smooth(method='lm', formula= y~x) +
  labs(y="Largest in-circle area (cm2)", x = expression(paste("Mean annual temperature ("^"o", "C)"))) +
  scale_y_continuous(trans='log10') +
  theme_bw() +
  theme (legend.position="none")
...
p2 <- ggplot (data = ref_result_RO, mapping = aes (x=Prec, y=circle_area_results)) +
  geom_jitter(aes(color=scientificName), alpha = 0.1, width = 0.1, height = 0.1, size = 0.2) +
  geom_smooth(method='lm', formula= y~x) +
  labs(y="Largest in-circle area (cm2)", x = "Mean annual precipitation (mm)") +
  scale_x_continuous(trans='log10') + scale_y_continuous(trans='log10') +
  theme_bw() +
  theme (legend.position="none")
...
p1 <- ggplot (data = ref_result_RO, mapping = aes (x=Temp, y=curvature_results)) +
  geom_jitter(aes(color=scientificName), alpha = 0.1, width = 0.1, height = 0.1, size = 0.2) +
  geom_smooth(method='lm', formula= y~x) +
  labs(y="Curvature ratio \n(leaf area/convex hull", x = expression(paste("Mean annual temperature ("^"o", "C)"))) +
  theme_bw() +
  theme (legend.position="none")
...
p2 <- ggplot (data = ref_result_RO, mapping = aes (x=Prec, y=curvature_results)) +
  geom_jitter(aes(color=scientificName), alpha = 0.1, width = 0.1, height = 0.1, size = 0.2) +
  geom_smooth(method='lm', formula= y~x) +
  labs(y="Curvature ratio \n(leaf area/convex hull", x = "Mean annual precipitation (mm)") +
  theme_bw() +
  scale_x_continuous(trans='log10') +
  theme (legend.position="none")
...
scatter3d(x=log10(ref_result_RO$Prec), y=log10(ref_result_RO$mask_area_results), z=ref_result_RO$Temp, 
          ...
          xlab="Log Mean annual precipitation (mm)", ylab = "Log leaf area size", zlab="Mean annual temperature (oC)")
...
overall_se <- summary(overall)$sigma
...
p1 <- ggplot() +
  geom_point(data = ref_result_RO, mapping = aes(y = mask_area_results, x = Prec), alpha = 0.05, color = "#E69F00", size = 0.7) + #try grey60?
...
#Attempt 4 - plots *mean* slope coefficient of each sp/sg/g
...
p1 <- ggplot (plot_results, aes(x= levels, y=summ_slp, group = 1)) +
  geom_point (size = 0.8) +
  geom_errorbar (aes(ymin=summ_slp-summ_se, ymax=summ_slp+summ_se), width=.2) +
  geom_label (aes(label = paste(formatC(summ_slp, format = "e", digits = 2), "+/-", formatC(summ_se, format = "e", digits = 2)), y=yloc), size = 3) +
  theme_bw() +
  xlab ("Phylogenetic level") + ylab ("Slope") +
  scale_x_discrete(limits = order, labels = c("Species", "Subgenus", "Genus", "Overall"))

ggsave(filename="~/Uni/Honours/Thesis/Data Analysis or Code/Plots/lmer_slopes_phylo.jpg", plot=p1, units = "px", height = 1500, width = 2000, limitsize = FALSE)
...
colnames(rsp_preds)[3] <- "masked_area_results"
...
p1 <- ggplot(ribbon = list(alpha = 0)) +
  ...
  geom_point (data = rsp_preds, aes(x=Prec, y=masked_area_results, color=scientificName), alpha=0.5) +
  #geom_line (data = rsp_preds, aes(x=Prec, y=masked_area_results, color=scientificName), alpha=0.5) +
  scale_x_continuous(trans='log10') +
...
colnames(rsg_preds)[3] <- "masked_area_results"
rsg_fixed_pred <- estimate_relation(rsg_glm) # This time, include_random is FALSE (default)

p2 <- ggplot(ribbon = list(alpha = 0)) +
  geom_point (data = rsg_preds, aes(x=(Prec), y=(masked_area_results), color=Subgenus), alpha=0.5) +
  geom_line (data = rsg_preds, aes(x=(Prec), y=(masked_area_results), color=Subgenus), alpha=0.5) +
...
colnames(rg_preds)[3] <- "masked_area_results"
rg_fixed_pred <- estimate_relation(rg_glm) # This time, include_random is FALSE (default)

p3 <- ggplot(ribbon = list(alpha = 0)) +
  #geom_point (data = ref_result_RO_rmspless, aes(x=log10(Prec), y=log10(mask_area_results), color=scientificName), alpha = 0.05) +
  geom_point (data = rg_preds, aes(x=Prec, y=masked_area_results, color=genus), alpha=0.5) +
  geom_line (data = rg_preds, aes(x=Prec, y=masked_area_results, color=genus), alpha=0.5) +
  scale_x_continuous(trans='log10') +
...
gt <-gridExtra::grid.arrange(gt, left = "Logged leaf area", bottom = "Mean annual precipitation (mm)") 

plot_all <- ggarrange(p1,p2,p3, ncol = 3)
plot_all

ggsave(filename="~/Uni/Honours/Thesis/Data Analysis or Code/Plots/lmer_slopes_phylo.jpg",plot=gt, units = "px", height = 1300, width = 3900, limitsize = FALSE)
...
#Plotting slopes of each clade against their number of data points
```{r Plotting slopes of each clade against their number of data points}
  # This code is similar to above, however carries out a lm from a filtered dataframe. Then from there it creates a data frame a of the coefficients of the lm at each level. This is repeated for species, subgenera and genus. This plots the slopes against each groups total data points

  # Carrying out for species
species_list <- as.list(unique(ref_result_RO_rmspless$scientificName))
...
ggplot(data = model_summ_MAR, aes(y=slope, x=observation_number)) +
 geom_point(size=2, aes(color = level), alpha = 0.2) +
 #geom_hline(yintercept=1.075, linetype="dashed", color = "red", size=1) + 
  labs(x="Number of observations per group", y = "Slope") +
  theme(aspect.ratio = 1, axis.line = element_line(size = 0.7, arrow=arrow(length = unit(0.2,"cm"), type = "closed"))) +
  xlim(c(0,4000)) +
  theme_minimal()
...
p2 <- ggplot(temp_values, aes(x=long_temp, y=lat_temp)) +
...
p3 <- ggplot(prec_values, aes(x=long_prec, y=lat_prec)) +
...
ggsave(filename="~/Uni/Honours/Thesis/Data Analysis or Code/Plots/Aust_prec-temp-distrib.jpg",plot=plot_all, units = "px", height = 1350, width = 4850, limitsize = FALSE)
```
...
write.csv(taxout, file = "~/Delete/cut_taxa.csv")
  ...
  #Reducing inclusivity criterion to prevent model convergence errors
ref_result_RO_rmspless <- ref_result_RO %>% #How to decide minimum?
  group_by(scientificName) %>% 
  mutate (count=n()) %>% 
  filter (count>25) %>% 
...
ggsave(filename="~/Uni/Honours/Thesis/Data Analysis or Code/Plots/Phylo_interval_plot.jpg",plot=p1, units = "px", height = 1500, width = 3000, limitsize = FALSE)
