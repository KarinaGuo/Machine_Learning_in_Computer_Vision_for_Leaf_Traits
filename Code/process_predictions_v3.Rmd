...
p1 <- ggplot (data = EucalyptsArea, mapping = aes (x=Prec, y=mask_area_results)) +
  ...
  labs(y="Leaf area (cm2)", x = "Mean annual precipitation (mm) (mm)") +
  theme_bw() +
  theme (legend.position="none") +
  scale_y_continuous(trans='log10')  +
  scale_x_continuous(trans='log10') + 
...
p1 <- ggplot(data=tmp_df_merge, aes(x =mask_area_results, y=..scaled.., fill = source)) +
  ...
  xlab(expression(paste("Leaf size (mm"^"2",")"))) +
  ylab("Density") +
  theme_bw() +
...
wrights_euc_lm_slp <- coef(wrights_euc_lm)[[2]]
wrights_euc_lm_se <- summary(wrights_euc_lm)$sigma
wrights_euc_lm_res <- cbind("Wright's euc", wrights_euc_lm_int, wrights_euc_lm_slp, aust_lm_se)
...
p1 <- ggplot(prec_values, aes(x=long_prec, y=lat_prec)) +
...
ggsave(filename="~/Uni/Honours/Thesis/Data Analysis or Code/Plots/Aust_prec-distrib.jpg",plot=p1, units = "px", height = 1450, width = 1850, limitsize = FALSE)
```
