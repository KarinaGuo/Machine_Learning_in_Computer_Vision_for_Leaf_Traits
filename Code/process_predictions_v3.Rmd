...
ggplot (QC_data, aes(x=mask_area_results)) +
  geom_histogram(bins = 30, aes(fill=QC), position="dodge")  +
  geom_freqpoly() +
  theme_bw() +
  facet_wrap(~QC)

ggplot (QC_data, aes(y=mask_area_results, x=QC)) +
  geom_boxplot() +
  theme_bw() 
...
comb_mean_Aust <- inner_join(EucalyptsArea_spsumm, ref_result_RO_spsumm, by = 'scientificName') %>% 
  mutate (diff = mean_AusTraits-mean_ML, source = "Aust") %>% 
  select(scientificName, diff, source)
comb_mean_Wright <- inner_join(ref_result_RO_spsumm, Wrights_data_spsumm, by = 'scientificName') %>% 
  mutate (diff = mean_Wright-mean_ML, source = "Wright") %>% 
  select(scientificName, diff, source)
...
tree_node_dated$tip.label <- gsub(" LAN.", "", tree_node_dated$tip.label)
