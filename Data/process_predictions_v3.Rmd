...
p1 <- ggplot (data = EucalyptsArea, mapping = aes (x=Temp, y=mask_area_results)) +
  geom_jitter(alpha = 0.1, width = 0.1, height = 0.1, size = 0.5) +
...
wrights_lm_slp <- coef(wrights_euc_lm)[[2]]

  # Slope of all of Wright's data points reported in SI
wrights_all_slp <- as.numeric(1.075)

  # Table!
head <- c("Overall slope", "Mean species slope", "Mixed model slope", "Austrait's euc' slope", "Wright's euc' slope", "Wright's all slope")
res <- c(lm_slp, sp_mean_lm_slp, lmer_slp, aust_lm_slp, wrights_lm_slp, wrights_all_slp)

lm_table <- as.data.frame(cbind(head, res))
lm_table
```
...
ggplot (loop_coefficient, aes(x=interval, y=summ_slp)) +
  geom_point() + 
